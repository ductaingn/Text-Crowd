...
class Roadmap:
    ...
    def build_roadmap_PRM(self, scenario_, sample_num=20, nb_dis=500):
        ...
        for key_i in scenario_.keys():
            ...
            for obj_id in range(len(scenario_[key_i])):
                obj_graph = copy.deepcopy(
                    scenario_[key_i][obj_id]["attributes"]["graph"]
                )
                if (
                    "points_idx_inRM"
                    not in scenario_expand[key_i][obj_id]["attributes"]["graph"].keys()
                ):
                    scenario_expand[key_i][obj_id]["attributes"]["graph"][
                        "points_idx_inRM"
                    ] = []
                if (
                    "edges_idx_inRM"
                ...
                for e_id in range(len(obj_graph["edges"])):
                    edges_.append(
                        [
                            obj_graph["edges"][e_id][0] + len(vertexes_),
                            obj_graph["edges"][e_id][1] + len(vertexes_),
                        ]
                    )
                    e_idx_inS.append(
                        {"key": copy.deepcopy(key_i), "obj_id": obj_id, "edge_id": e_id}
                    )
                    scenario_expand[key_i][obj_id]["attributes"]["graph"][
                        "edges_idx_inRM"
                    ].append(len(edges_) - 1)
                for p_id in range(len(obj_graph["points"])):
                    ...
                    scenario_expand[key_i][obj_id]["attributes"]["graph"][
                        "points_idx_inRM"
                    ].append(len(vertexes_) - 1)
                obj_poly_list.append(
                    geom.Polygon(
                        [[p[0], p[1]] for p in copy.deepcopy(obj_graph["graph_box"])]
                    )
                )
        ...
        scenario_expand["roadmap"] = {
            "vertexes": copy.deepcopy(vertexes_),
    ...
    def sample_path(self, roadmap, constrains):
        ...
        for eg_id in range(len(roadmap["edges"])):
            graph_[roadmap["edges"][eg_id][0]].append(
                {"v": roadmap["edges"][eg_id][1], "edge_id": eg_id}
            )
        ...
        while st_:
            ...
            for nvid, nv_i in enumerate(nxt_vs):
                if nv_i["v"] in path_v:
                    continue
                if (
                    roadmap["vertexes_idx_inS"][nv_i["v"]] is not None
                    and roadmap["vertexes_idx_inS"][nv_i["v"]]["key"] == "areas_list"
                    and nv_i["v"] != constrains["goal_p"]
                ):
                    continue
                if len(path_e) != 0:
                    pre_e = [
                        copy.deepcopy(roadmap["vertexes"][path_e[-1]["edge"][1]]),
                        copy.deepcopy(roadmap["vertexes"][path_e[-1]["edge"][0]]),
                    ]
                    cur_e = [
                        copy.deepcopy(roadmap["vertexes"][v_]),
                ...
                for eg_pre_i in copy.deepcopy(path_e)[:-1]:
                    ...
                    eg_ = [
                        copy.deepcopy(roadmap["vertexes"][eg_pre_i["edge"][0]]),
