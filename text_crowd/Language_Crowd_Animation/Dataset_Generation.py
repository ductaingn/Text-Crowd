...
parameters_simple = {
    ...
    "roadmap_sample_n": 0,
    "roadmap_nb_dis": 2048,
    "path_constrains_default": {
        "start_p": None,
        "goal_p": None,
        "line_angle_lim": 89,
        "adaptive_w": {"v_w": 0.95, "e_w": 0.7},
        "safe_dis": 100,
        "path_len_range": [1, 9],
    },
    "path_relax_dis": 30,
...
def dataset_generation(args):
    param_ = (
        copy.deepcopy(parameters_simple)
        if args.mode == "simple"
        else copy.deepcopy(parameters_hard)
    )
    SG_ = Scenario_Generator()
    RM_ = Roadmap()
    BD_ = Behavior_Descriptor(
        snr_mid_scl=param_["mid_scale"],
        snr_inner_scl=param_["inner_scale"],
        global_adj_num=param_["global_adj_num"],
        local_adj_num=param_["local_adj_num"],
        dir_adj_num=param_["dir_adj_num"],
        drop_out_ps=copy.deepcopy(param_["drop_out_ps"]),
    )
    assert param_["field_use"] == "CTF"
    FG_ = CurveTracking_Field(
        reverse_direction=param_["reverse_direction"],
        vr=param_["vr"],
        kf=param_["kf"],
        flag_follow_obstacle=param_["flag_follow_obstacle"],
        epsilon=param_["epsilon"],
        switch_dist_0=param_["switch_dist_0"],
        switch_dist=param_["switch_dist"],
        lidar_N=param_["lidar_N"],
    )

    for obj_n in param_["obj_nums"]:
        for group_n in param_["group_nums"]:
            print(
                "### Generating data for obj_num =", obj_n, "and group_num =", group_n
            )
            case_id = 0
            dataset_obj_group = {
            ...
            while case_id < args.dataset_size_per_case:
                ...
                while 1:
                    ...
                    snr_param["objects"]["entrance"]["num"] = random.randint(
                        group_n, param_["area_max_num"]
                    )
                    snr_param["objects"]["exit"]["num"] = random.randint(
                        1, param_["area_max_num"]
                    )
                ...
                scenario_, roadmap_ = RM_.build_roadmap_PRM(
                    scenario_=copy.deepcopy(snr_init),
                    sample_num=param_["roadmap_sample_n"],
                    nb_dis=param_["roadmap_nb_dis"],
                )
                ...
                for group_id in range(group_n):
                    ...
                    exit_id = snr_param["objects"]["entrance"]["num"] + random.randint(
                        0, snr_param["objects"]["exit"]["num"] - 1
                    )
                    start_p = copy.deepcopy(
                        scenario_["areas_list"][etc_id]["attributes"]["graph"][
                            "points_idx_inRM"
                        ][0]
                    )
                    goal_p = copy.deepcopy(
                        scenario_["areas_list"][exit_id]["attributes"]["graph"][
                            "points_idx_inRM"
                        ][0]
                    )
                    ...
                    path_vs_init, path_es_init = RM_.sample_path(
                        roadmap=copy.deepcopy(scenario_["roadmap"]),
                        constrains=copy.deepcopy(constrains_),
                    )
                    if path_vs_init is None or path_es_init is None:
                        valid_path = False
                        break
                    path_vs, path_es = RM_.path_postprocess(
                        scenario_=copy.deepcopy(scenario_),
                        path_v=copy.deepcopy(path_vs_init),
                        path_e=copy.deepcopy(path_es_init),
                        rlx_dis=param_["path_relax_dis"],
                    )
                    groups_paths.append(
                        [copy.deepcopy(path_vs), copy.deepcopy(path_es)]
                    )

                    # get the language description of the path
                    behavior_ = BD_.path_to_descriptor(
                        copy.deepcopy(scenario_),
                        copy.deepcopy(path_vs),
                        copy.deepcopy(path_es),
                    )
                    ...
                    description_ = BD_.descriptor_to_text(
                        dsc_l=copy.deepcopy(behavior_),
                        dropout_ps=copy.deepcopy(param_["drop_out_ps"]),
                    )
                ...
                smap_ = SG_.get_semantic_map(
                    scenario_in=copy.deepcopy(scenario_),
                    grid_width=param_["grid_width_map"],
                )
                ...
                for g_id in range(len(groups_paths)):
                    ...
                    sg_areas = {
                        "start_area": scenario_["roadmap"]["vertexes_idx_inS"][s_vid_][
                            "obj_id"
                        ],
                        "goal_area": scenario_["roadmap"]["vertexes_idx_inS"][g_vid_][
                            "obj_id"
                        ],
                    }
                    sg_distr = SG_.get_sg_distb(
                        scenario_in=copy.deepcopy(scenario_),
                        grid_width=param_["grid_width_map"],
                ...
                group_fields = []
                FG_.reset(
                    scenario=copy.deepcopy(scenario_),
                    grid_width=param_["grid_width_field"],
                )
                for g_id in range(len(groups_paths)):
                    lines = []
                    for e_i in groups_paths[g_id][1]:
                        lines.append(
                            [
                                copy.deepcopy(
                                    scenario_["roadmap"]["vertexes"][e_i["edge"][0]]
                                ),
                                copy.deepcopy(
                                    scenario_["roadmap"]["vertexes"][e_i["edge"][1]]
                                ),
                            ]
                        )
                    ...
                    field_ = FG_.get_field(
                        guidance=copy.deepcopy(guidance_),
                        constrains=copy.deepcopy(field_constrains),
                    )
                ...
                dt_i = {
                    ...
                    "obj_num": obj_n,
                    "group_num": group_n,
                    "group_descriptors": groups_descriptors,
                    "group_descriptions": groups_descriptions,
                    "semantic_map": smap_,
                    "group_sizes": groups_sizes,
                    "group_sg_distrs": groups_distrs,
                    "group_paths": groups_paths,
            ...
            scd_p = int(
                len(dataset_obj_group["data"])
                * (args.dataset_proportion[0] + args.dataset_proportion[1])
            )
            dt_training = {}
            dt_validation = {}
            dt_testing = {}
            for key_i in dataset_obj_group.keys():
                if key_i == "data":
                    dt_training[key_i] = copy.deepcopy(
                        dataset_obj_group[key_i][0:fst_p]
                    )
                    dt_validation[key_i] = copy.deepcopy(
                        dataset_obj_group[key_i][fst_p:scd_p]
                    )
...
def field_control_visualization(
    scenario,
    group_distrs,
    group_paths,
    group_sizes,
    ...
    for gid in range(group_n):
        ...
        st_poses = random.choices(
            list(range(len(gi_st_distr))), weights=gi_st_distr, k=group_sizes[gid]
        )
        g_poses = random.choices(
            list(range(len(gi_g_distr))), weights=gi_g_distr, k=group_sizes[gid]
        )
        for gaid in range(group_sizes[gid]):
            ai_params = copy.deepcopy(AGNET_PARAM_DEFAULT)
            ai_params["pos"] = [
                int(st_poses[gaid] / grid_size[0]) * grid_width
                + grid_width / 2
                + random.uniform(0, 1),
                int(st_poses[gaid] % grid_size[0]) * grid_width
                + grid_width / 2
            ...
            ai_params["goal_pos"] = [
                int(g_poses[gaid] / grid_size[0]) * grid_width
                + grid_width / 2
                + random.uniform(0, 1),
                int(g_poses[gaid] % grid_size[0]) * grid_width
                + grid_width / 2
        ...
        if show_path:
            for e_i in group_paths[gid][1]:
                lines.append(
                    [
                        copy.deepcopy(scenario["roadmap"]["vertexes"][e_i["edge"][0]]),
    ...
    fld_env.reset(
        scenario=copy.deepcopy(scenario), agent_setting=copy.deepcopy(agent_params)
    )
    if show_path:
        for gid, (group_path_v_i, group_path_e_i) in enumerate(group_paths):
            ...
            for edge_i in group_path_e_i:
                fld_env.viewer.add_line(
                    p1=scenario["roadmap"]["vertexes"][edge_i["edge"][0]],
                    p2=scenario["roadmap"]["vertexes"][edge_i["edge"][1]],
                    color=color_i,
                )
    while 1:
        fld_env.render()
    ...
    while 1:
        ...
        if keyboard[pyglet.window.key.Q]:
            fld_env.set_viewer_field(
                gfields_for_ctrl[0]["grid"], gfields_for_ctrl[0]["field"]
            )
        elif keyboard[pyglet.window.key.W]:
            fld_env.set_viewer_field(
                gfields_for_ctrl[1]["grid"], gfields_for_ctrl[1]["field"]
            )
        elif keyboard[pyglet.window.key.E]:
            fld_env.set_viewer_field(
                gfields_for_ctrl[2]["grid"], gfields_for_ctrl[2]["field"]
            )
...
def dataset_visual(args, data_path):
    ...
    for dt_id in range(len(data_["texts"])):
        ...
        map_colors = np.concatenate(
            [np.random.randint(0, 255, (len(smap_[0][0]) - 2, 3)), sg_colors], axis=0
        )
        smap_cvimg = cv_visual_map(
            map_in=copy.deepcopy(smap_), colors=map_colors, save_nm=None
        )
        distr_cvimg = cv_visual_map(
            map_in=copy.deepcopy(sg_distr), colors=sg_colors, save_nm=None
        )
        ...
        orca_env.reset(
            scenario=copy.deepcopy(scenario_),
            agent_setting={
                "init_agent_params": copy.deepcopy(orca_env.agent_current_infor)
            },
        )
        # plot the roadmap
        for vid, v_i in enumerate(scenario_["roadmap"]["vertexes"]):
            v_color = (
                [255, 0, 0]
                if scenario_["roadmap"]["vertexes_idx_inS"][vid] is not None
                else [0, 0, 0]
            )
        ...
        for eid, e_i in enumerate(scenario_["roadmap"]["edges"]):
            e_color = (
                [0, 255, 0]
                if scenario_["roadmap"]["edges_idx_inS"][eid] is not None
                else [90, 90, 90]
            )
            orca_env.viewer.add_line(
                p1=scenario_["roadmap"]["vertexes"][e_i[0]],
                p2=scenario_["roadmap"]["vertexes"][e_i[1]],
                color=e_color,
            )
        for gid, (group_path_v_i, group_path_e_i) in enumerate(paths_):
            ...
            for edge_i in group_path_e_i:
                orca_env.viewer.add_line(
                    p1=scenario_["roadmap"]["vertexes"][edge_i["edge"][0]],
                    p2=scenario_["roadmap"]["vertexes"][edge_i["edge"][1]],
                    color=color_i,
                )
        while 1:
            orca_env.render()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset_proportion", default=[0.6, 0.2, 0.2]
