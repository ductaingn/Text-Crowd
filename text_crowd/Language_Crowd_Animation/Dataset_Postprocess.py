...
class Postprocess_Config:
    ...
    data_path = "/home/linh/ductai_nguyen_ws/Text-Crowd/text_crowd/Language_Crowd_Animation/Dataset/Data_Full_V1/"
    obj_nums = [0, 1, 2, 3, 4, 5]
    group_nums = [1, 2, 3]
    data_category = "training"

    target_path = "/home/linh/ductai_nguyen_ws/Text-Crowd/text_crowd/Language_Crowd_Animation/Dataset/Data_Full_V2/"
...
def dataset_generation_v2(config):
    ...
    for obj_n in config.obj_nums:
        for group_n in config.group_nums:
            ...
            dt_path_og = (
                config.data_path
                + f"Obj{obj_n}_Group{group_n}/dt_{config.data_category}.npy"
            )
            ...
            for data_id in range(len(data_v1_og["data"])):
                for group_id in range(data_v1_og["data"][data_id]["group_num"]):
                    ...
                    data_target_og["data"][data_id]["group_fields"][group_id] = (
                        field_postprocess(
                            scenario=copy.deepcopy(
                                data_v1_og["data"][data_id]["scenario"]
                            ),
                            sgdistrs=copy.deepcopy(
                                data_v1_og["data"][data_id]["group_sg_distrs"][group_id]
                            ),
                            paths=copy.deepcopy(
                                data_v1_og["data"][data_id]["group_paths"][group_id]
                            ),
                            initial_field=copy.deepcopy(
                                data_v1_og["data"][data_id]["group_fields"][group_id]
                            ),
...
def field_postprocess(scenario, sgdistrs, paths, initial_field, config):
    ...
    while 1:
        ...
        group_field_dict = {
            "agent_ids": list(range(config.crowd_size)),
        ...
        st_poses = random.choices(
            list(range(len(st_distr))), weights=st_distr, k=config.crowd_size
        )
        for agt_id in range(config.crowd_size):
            agent_params["init_agent_params"][agt_id]["pos"] = [
                int(st_poses[agt_id] / grid_size[0]) * grid_width
                + grid_width / 2
                + random.uniform(-config.pos_rand_range, config.pos_rand_range),
                int(st_poses[agt_id] % grid_size[0]) * grid_width
                + grid_width / 2
                + random.uniform(-config.pos_rand_range, config.pos_rand_range),
            ]
            agent_params["init_agent_params"][agt_id]["goal_pos"] = copy.deepcopy(
                goal_point
            )
        ...
        while 1:
            if config.visual:
                ...
                if keyboard[pyglet.window.key.Q]:
                    fld_env.set_viewer_field(
                        group_field_dict["grid"], group_field_dict["field"]
                    )
            ...
            for aid in group_field_dict["agent_ids"]:
                if (
                    ...
                    < config.reach_dis
                    or fld_env.agent_current_infor[aid]["pos"][0] <= 1e-6
                    or fld_env.agent_current_infor[aid]["pos"][0]
                    >= scenario["wind_size"][0] - 1e-6
                    or fld_env.agent_current_infor[aid]["pos"][1] <= 1e-6
                    or fld_env.agent_current_infor[aid]["pos"][1]
                    >= scenario["wind_size"][1] - 1e-6
                ):
            ...
            pre_positions_grp = np.array(pre_agent_positions)[
                group_field_dict["agent_ids"]
            ]
            aft_positions_grp = np.array(aft_agent_positions)[
                group_field_dict["agent_ids"]
            ]
            ...
            agt_grid_idxs[:, 0][agt_grid_idxs[:, 0] > (grid_size[0] - 1)] = (
                grid_size[0] - 1
            )
            agt_grid_idxs[:, 1][agt_grid_idxs[:, 1] > (grid_size[1] - 1)] = (
                grid_size[1] - 1
            )
            # get the gradient map
            grads_ = np.array(aft_positions_grp) - np.array(pre_positions_grp)
            grads_ = np.nan_to_num(
                grads_ / (np.linalg.norm(grads_, axis=1).reshape((-1, 1))), 0.0
            )  # normalization
            ...
            curr_field_norm = curr_field_norm.reshape(
                (curr_field_norm.shape[0], curr_field_norm.shape[1], 1)
            )
        ...
        total_aver_grads = np.sum(
            np.linalg.norm(
                np.array(current_field) - np.array(field_before_update), axis=2
            )
        ) / np.sum(upd_map)

        print(
            f"iteration {iter_cnt} | simulated steps: {step} | total_aver_grads: {total_aver_grads}"
    ...
    current_field[obs_idx[:, 0], obs_idx[:, 1]] = np.array([0.0, 0.0])
...
def data_visual(config, obj_n, group_n, rand_data_num=10):
    ...
    data_og = np.load(
        config.target_path + f"Obj{obj_n}_Group{group_n}/dt_{config.data_category}.npy",
        allow_pickle=True,
    ).item()
    ...
    for i in range(rand_data_num):
        ...
        cv_visual_map(
            map_in=data_for_visual["semantic_map"], colors=None, save_nm=None, show=True
        )

        field_control_visualization(
            scenario=data_for_visual["scenario"],
            group_distrs=data_for_visual["group_sg_distrs"],
            group_paths=data_for_visual["group_paths"],
            group_sizes=data_for_visual["group_sizes"],
            group_fields=data_for_visual["group_fields"],
            show_path=True,
            show_fields=True,
        )
