...
class Scenario_Generator:
    ...
    def random_scenario(self, scenario_configs):
        ...
        boundary = geom.LineString(
            geom.Polygon(
                get_box_ll(
                    x=wind_size[0] * (1 - srk_scale),
                    y=wind_size[1] * (1 - srk_scale),
                    lowerleft=(w_rg[0], h_rg[0]),
                )
            ).exterior.coords
        ...
        for obj_name in scenario_configs["objects"].keys():
            ...
            obj_size_range = copy.deepcopy(
                scenario_configs["objects"][obj_name]["size_range"]
            )
            for r_i in range(obj_num):
                    ...
                    if obj_name == "entrance" or obj_name == "exit":
                        ...
                        for a_i in scenario_["areas_list"]:
                            ...
                            dis_lim = max(
                                ...
                                / 2
                                + scenario_configs["objects"][obj_name]["size_range"][1]
                                / 2,
                            )
                            if (
                                np.linalg.norm(
                ...
                obj_graph_ps, obj_graph_egs, obj_graph_box = self.get_obj_graph(
                    copy.deepcopy(obj),
                    scenario_configs["objects"][obj_name]["graph_buffer"],
                )
    ...
    def get_rand_obj(self, obj_type, objsize_range, area_):
        ...
        if obj_type == "rectangle":
            ...
            r_, ag_ = (
                random.uniform(objsize_range[0], objsize_range[1]) / 2,
