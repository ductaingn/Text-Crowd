...
class Navigation_Field(Base_Field):
    ...
    def get_field(self, guidance, constrains):
        ...
        if constrains["with_goal"]:
            ...
            nav_field = self.get_navigation_field(
                guidance_field, constrains["goal_position"]
            )
            return nav_field
        else:
        ...
        if guidance_source["type"] == "lines":
            ...
            for line_i in lines:
                ...
                for p_i in inter_ps:
                    ...
                    intersections_ = np.array(
                        box_poly.intersection(geom.LineString(line_i)).coords
                    )
                    if len(intersections_) < 2:
                        continue
                    dir_linei = (
                        np.array(line_i) - np.array(line_i[0])
                    ) / np.linalg.norm(np.array(line_i) - np.array(line_i[0]))
                    dir_inter = (
                        np.array(intersections_) - np.array(intersections_[0])
                    ) / np.linalg.norm(
                        np.array(intersections_) - np.array(intersections_[0])
                    )
                    ...
                    intersections_ = (
                        intersections_ - intersections_[0]
                    ) / np.linalg.norm(intersections_ - intersections_[0])
            ...
            for pt in current_field_points:
                ...
                guidance_field[pt[0]][pt[1]] = np.array(
                    guidance_field[pt[0]][pt[1]]
                ) / np.linalg.norm(np.array(guidance_field[pt[0]][pt[1]]))

            # expand
            if self.g_handle_colli:
                lidar = Sensor(
                    agent_r=0,
                    obs_all=ORCA_Env.get_all_obstacles_from_scenario(scenario_),
                    sensorRange=self.obs_range * 2,
                    N=self.lidar_N,
                )
                ...
                for pt in current_field_points:
                    for x_dr, y_dr in [[-1, 0], [0, -1], [0, 1], [1, 0]]:
                        ...
                        if (
                            (
                                nb_x < 0
                                or nb_x >= len(grid_infor)
                                or nb_y < 0
                                or nb_y >= len(grid_infor[0])
                                or grid_infor[nb_x][nb_y]["free"] == 1
                            )
                            or visited_map[nb_x][nb_y] == 1
                            or [nb_x, nb_y] in next_field_points
                        ):
                            continue
                        ...
                        for x_dr_, y_dr_ in [[-1, 0], [0, -1], [0, 1], [1, 0]]:
                            ...
                            if (
                                nbnb_x < 0
                                or nbnb_x >= len(grid_infor)
                                or nbnb_y < 0
                                or nbnb_y >= len(grid_infor[0])
                        ...
                        if self.g_handle_colli:
                            lidar_infor = lidar.get_sensor_reading(
                                [copy.deepcopy(grid_infor[nb_x][nb_y]["center"])]
                            )[0]
                            ...
                            if min_dis < self.obs_range:
                                ...
                                if np.dot(dir_, ori_vec) > 0:
                                    ori_vec -= self.obs_repel_scale * (
                                        dir_
                                        / np.linalg.norm(dir_)
                                        * (1 - min_dis / self.obs_range)
                                    )
                                    vec = (
                                        ori_vec
                                        / np.linalg.norm(ori_vec)
                                        * (decay_rate ** (iter + 1))
                                    )
    ...
    def get_navigation_field(self, guidance_field, goal_position):
        ...
        goal_grid = [
            int(goal_position[0] / grid_width),
            int(goal_position[1] / grid_width),
        ]
        ...
        while 1:
            current_cost_list = cost_map[
                np.array(current_visit_list)[:, 0], np.array(current_visit_list)[:, 1]
            ]
            ...
            for x_dr, y_dr in [[-1, 0], [0, -1], [0, 1], [1, 0]]:
                ...
                if (
                    neighbor_x < 0
                    or neighbor_x >= len(grid_map)
                    or neighbor_y < 0
                    or neighbor_y >= len(grid_map[0])
                ...
                if visited_map[neighbor_x, neighbor_y] == 0:
                    ...
                    for x_dr_, y_dr_ in [[-1, 0], [0, -1], [0, 1], [1, 0]]:
                        ...
                        if (
                            nb_nb_x < 0
                            or nb_nb_x >= len(grid_map)
                            or nb_nb_y < 0
                            or nb_nb_y >= len(grid_map[0])
                    ...
                    update_map[neighbor_x, neighbor_y] = max(
                        update_map[neighbor_x, neighbor_y], len(nb_nbs)
                    )
                    ### compute cost for this neighbor node
                    if len(nb_nbs) == 1 or (
                        len(nb_nbs) == 2
                        and np.linalg.norm(
                            np.array(nb_nbs_dirs[0]) + np.array(nb_nbs_dirs[1])
                        )
                        < 1e-6
                    ):
                        for nbnb_id in range(len(nb_nbs)):
                            ...
                            A_G_cross = np.cross(
                                np.array(dir_A),
                                np.array(guidance_field[neighbor_x][neighbor_y]),
                            )
                            if abs(A_G_cross) < 1e-6:
                                ...
                                for dir_b in [[-1, 0], [0, -1], [0, 1], [1, 0]]:
                                    if (
                                        abs(
                                            np.cross(np.array(dir_A), np.array(dir_b))
                                            * A_G_cross
                                        )
                                        > 1e-6
                                    ):
                            ...
                            if res.fun < cost_map[neighbor_x, neighbor_y]:
                                ...
                                vec_ = np.array(
                                    res.x[0] * np.array(nb_nbs_dirs[nbnb_id])
                                )
                                navigation_field[neighbor_x, neighbor_y] = np.array(
                                    vec_
                                ) / np.linalg.norm(np.array(vec_))
                    else:
                        for nb1_id in range(len(nb_nbs)):
                            for nb2_id in range(nb1_id + 1, len(nb_nbs)):
                                if (
                                    np.linalg.norm(
                                        np.array(nb_nbs_dirs[nb1_id])
                                        + np.array(nb_nbs_dirs[nb2_id])
                                    )
                                    < 1e-6
                                ):
                                    continue
                                ...
                                res = minimize(
                                    ...
                                    (
                                        ...
                                        dir_B,
                                        np.array(
                                            guidance_field[neighbor_x][neighbor_y]
                                        ),
                                    ),
                                    bounds=[(0, 1)],
                                )
                                if res.fun < cost_map[neighbor_x, neighbor_y]:
                                    cost_map[neighbor_x, neighbor_y] = res.fun
                                    vec_ = np.array(
                                        res.x[0] * np.array(nb_nbs_dirs[nb1_id])
                                        + (1 - res.x[0]) * np.array(nb_nbs_dirs[nb2_id])
                                    )
                                    navigation_field[neighbor_x, neighbor_y] = np.array(
                                        vec_
                                    ) / np.linalg.norm(np.array(vec_))
    ...
    def cost_function_full(alpha, costA, costB, dir_A, dir_B, G_):
        ...
        return (
            alpha * costA
            + (1 - alpha) * costB
            + np.linalg.norm(np.array([alpha, 1 - alpha])) / s
        )
...
if __name__ == "__main__":
    ...
    constrains = {"with_goal": False, "goal_position": None}
    nav_fld = Navigation_Field(
        guidance_handle_colli=True, obs_inful_range=60, lidar_N=128, obs_repel_scale=1.3
    )
