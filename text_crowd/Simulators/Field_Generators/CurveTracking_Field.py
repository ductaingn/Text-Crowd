...
import copy
from .vectorfield_stack.distance_field import (
    distancefield_class,
)
...
# switch_dist_ # distance from which the robot will start to follow the obstacle
...
class CurveTracking_Field(Base_Field):
    ...
    def reset_params(
        ...
        # set vec field obj
        self.vec_field_obj = distancefield_class(
            self.vr,
            self.kf,
    ...
    def get_field(
        ...
        line_lengths = np.linalg.norm(
            np.array(guidance["params"]["lines"])[:, 1]
            - np.array(guidance["params"]["lines"])[:, 0],
            axis=1,
        )
        ...
        path_points.append(
            (
                guidance["params"]["lines"][-1][-1][0],
                guidance["params"]["lines"][-1][-1][1],
                0.0,
            )
        )
        ...
        if self.flag_follow_obstacle:
            lidar_ = Sensor(
                agent_r=0,
                obs_all=ORCA_Env.get_all_obstacles_from_scenario(self.scenario),
